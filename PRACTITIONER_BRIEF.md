...
We re-ran this same fairness test with a different kind of detector entirely — not our own scoring rules, but a small local AI model measuring how "surprised" it is by the text (a published academic technique called Binoculars, run with smaller substitute models to fit our hardware). It reached the same verdict, and by a wider margin: 0.53 on the same 0.50-to-1.0 scale, weaker than our own detector's 0.60. Two different kinds of detector agree this is a weak, not a strong, ELL predictor.
...
**A far cruder trick protects honest students more than it helps cheaters.** We also tested a much simpler edit than a paraphrasing tool: mechanically varying sentence length and swapping in contractions — the kind of tip a student could get from a one-paragraph "what detectors look for" list, no software required. It barely moved the AI catch rate (52% to 50%, not a reliable change either way) — but it roughly cut the false-positive rate on genuinely human essays in half (37% to 20%, a real drop). The one piece of advice a student could actually follow by hand did far more to protect innocent writers from false accusations than it did to help anyone cheat.
